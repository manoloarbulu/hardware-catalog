...
export const ProductMaintenance = () => {
  ...
  const openCreate = () => {
    ...
    setForm({
      ...emptyProduct(),
      category: category ?? ProductCategory.Processor,
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (!Number.isInteger(form.value) || form.value <= 0) {
      setMessage('Value must be a whole number greater than 0.');
      return;
    }
    ...
    try {
      ...
      setMessage('Unable to save product. Verify all fields are complete.');
    }
  };
  ...
  const selectCategory = (value: ProductCategory) => {
    ...
    <div className="space-y-5">
      ...
      <div ref={pickerRef} className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="relative">
          ...
          <button
            type="button"
            onClick={() => setFilterPicker(filterPicker === 'category' ? null : 'category')}
            className="mt-2 w-full rounded border border-gray-300 bg-white px-3 py-2 text-left"
          >
            {category ?? 'Select a category'}
          </button>
          {filterPicker === 'category' && (
            <div className="absolute z-10 mt-1 w-full border border-gray-300 bg-white shadow-lg">
              {Object.values(ProductCategory).map((item) => (
                <button
                  type="button"
                  key={item}
        ...
        <div className="relative">
          <label className="block font-semibold">Filter by Brand</label>
          <button
            type="button"
            onClick={() => setFilterPicker(filterPicker === 'brand' ? null : 'brand')}
            className="mt-2 w-full rounded border border-gray-300 bg-white px-3 py-2 text-left"
          >
            {brands.find((brand) => brand.id === brandId)?.name ?? 'Select a brand'}
          </button>
          {filterPicker === 'brand' && (
            <div className="absolute z-10 mt-1 max-h-52 w-full overflow-y-auto border border-gray-300 bg-white shadow-lg">
              ...
              {filterBrands.length === 0 && (
                <p className="px-3 py-2 text-sm text-gray-500">No brands in this category.</p>
              )}
            </div>
          )}
        </div>
      ...
      {!category && !brandId ? (
        <p className="text-gray-600">Select a category, brand, or both to list products.</p>
      ) : (
      ...
      {formOpen && (
        <form
          onSubmit={save}
          className="grid grid-cols-1 gap-3 border border-gray-300 bg-white p-4 md:grid-cols-2"
        >
          ...
          <label className="block font-semibold">
            ...
            >
              {Object.values(ProductCategory).map((item) => <option key={item}>{item}</option>)}
            </select>
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block font-semibold">
              ...
              <select
                ...
                className="mt-1 w-full rounded border border-gray-300 px-3 py-2"
              >
                {Object.values(UnitOfMeasure).map((item) => <option key={item}>{item}</option>)}
              </select>
            </label>
            <label className="block font-semibold">
              Value
              <input
                ...
                step={1}
                value={form.value}
                onChange={(event) => setForm({ ...form, value: parseInt(event.target.value, 10) })}
                className="mt-1 w-full rounded border border-gray-300 px-3 py-2"
                required
              />
            </label>
